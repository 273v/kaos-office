...
These standards apply to Python code in `kaos-office`.

`kaos-office` publishes the `kaos_office` import package. This package publishes CLI entry point(s): `kaos-office`, `kaos-office-serve`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-office` publishes import package `kaos_office`.
- Declare the public API in `kaos_office.__all__` when the package has
...
- Names exported from `kaos_office.__all__`.
...
- Put optional integrations in named extras. Public extras currently declared: `nlp`, `pptx`, `xlsx`, `xlsx-calamine`, `xlsx-formulas`.
