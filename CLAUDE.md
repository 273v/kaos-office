...
- **Phase 6.2** (SDT / content-control preservation): `<w:sdt>` wrappers are now preserved across a round-trip. Reader (`_parse_sdt_pr`) captures the control's metadata (`tag`, `alias`, `id`, `lock`, `control_type` ∈ {`text`/`richText`/`picture`/`date`/`checkbox`/`comboBox`/`dropDownList`/`docPartObj`/`docPartList`/`group`}, `showing_placeholder`) into `Attr.kv` under an `sdt.*` namespace and wraps the inner content in a `Div(classes=("sdt",))` (block-level) or `Span(classes=("sdt",))` (inline). Writer (`_serialize_sdt_block` / `_serialize_sdt_inline`) detects the class and re-emits `<w:sdt><w:sdtPr>...</w:sdtPr><w:sdtContent>...</w:sdtContent></w:sdt>` with the same metadata. Pre-6.2 the reader unwrapped SDTs silently, so Word templates with form controls or protected regions devolved to plain text on every KAOS pass. Deliberately NOT preserved: `<w:dataBinding>` (references `/customXml/item*.xml` parts we don't yet carry) and `<w:placeholder><w:docPart/>` (references `/word/glossary/document.xml`) — Word degrades these gracefully rather than red-boxing a dangling reference. Per-control-kind properties (dateFormat, dropdown list items, etc.) are also not yet preserved; the control retains its type identity but loses validation rules.
- **Phase 6.3** (PPTX text overflow policy): `write_pptx(..., overflow="warn"|"autofit"|"extend")` controls how the writer handles text that may not fit its shape. Pre-6.3 the writer set no auto-size and PowerPoint silently cropped LLM-generated summary slides. `"warn"` (default) leaves auto-sizing off but emits a logger warning via `_warn_if_probable_overflow` when character density exceeds 150 chars/sq in of shape area — conservative so false positives beat silent truncation. `"autofit"` maps to `MSO_AUTO_SIZE.TEXT_TO_FIT_SHAPE` (PowerPoint shrinks the font). `"extend"` maps to `MSO_AUTO_SIZE.SHAPE_TO_FIT_TEXT` (PowerPoint grows the shape). Both auto-size modes emit the corresponding DrawingML element (`<a:normAutofit/>` / `<a:spAutoFit/>`) verified in tests. Threading: `write_pptx` → `write_pptx_bytes` → `_add_slide` → `_fill_body_placeholder` / `_add_body_textbox`.
