...
| Status report.pptx | Status-report style template — original `docProps/core.xml` was absent (parser exercises the no-core-props code path), so creator/title/last-modified are not recorded in the file metadata. Ported from `kelvin_office/tests/resources/pptx/Status report.pptx`. | `<unknown — needs verification>` (likely a public Microsoft Office "status report" template; license not asserted) | 2026-04-03 | 27331690733357b8eddbc88b5860699663a4e646bb85bcf59af069797112ba7e | Exercises the reader's missing-core-props path on a real-sized deck. |
...
- `Status report.pptx` — recover the upstream URL / template source if
  one exists; the missing `docProps/core.xml` makes provenance harder
  to pin down. If no public source can be confirmed, replace with an
  equivalent hand-crafted minimal-no-core-props fixture (similar to
  `stress/no-core-props.pptx`).
