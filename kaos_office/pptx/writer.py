...
def _segment_into_slides(body: tuple) -> list[list]:
    ...
    try:
        ...
        explicit_divs = [
            b for b in body if isinstance(b, Div) and "slide" in (getattr(b, "classes", ()) or ())
        ]
    except ImportError:
        explicit_divs = []
...
def _add_slide(prs: Presentation, blocks: list) -> None:
    ...
    for block in blocks:
        ...
        if block_type == "Div":
            classes = getattr(block, "classes", ()) or ()
    ...
    if speaker_notes is not None:
        notes_text = _extract_text(speaker_notes)
        if notes_text.strip():
...
def _add_body_textbox(slide: Any, blocks: list) -> None:
    ...
    text_blocks = [b for b in blocks if not isinstance(b, Table)]
